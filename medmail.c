...
void listerMailRecus();
void listerMailEnvoyes();
...
void reponseMail(email *e)
{

  FILE *sourceBdd, *motsClesBdd;
  sourceBdd = fopen("log_email_out.dat", "ab");
  motsClesBdd = fopen("motcles.dat", "rb");
  if (sourceBdd == NULL || motsClesBdd == NULL)
  {
  ...
  char reponsePart1[10],reponsePart2[15],reponsePart3[21],reponsePart4[98];
  strcpy(reponsePart1, "\nBonjour,");
  strcpy(reponsePart2, "\n\nPour votre ");
  strcpy(reponsePart3, " je vous recommande ");
  strcpy(reponsePart4, ".\nN'hesitez pas à prendre rendez vous si le problème persiste.\n\nCordialement, Dr. MedMail.\n");

  strcpy(reponse.email, "repondeur@medmail.com");
  strcpy(reponse.obj, "RE :  ");
  strcat(reponse.obj, e->obj);

  bool found = false;

  while(fread(&motsCles, sizeof(motsCles), 1, motsClesBdd))
  {
    if (strstr(motsCles.mot, e->corps) != NULL || strstr(motsCles.mot, e->obj) != NULL)
    {
      found = true;
      strcpy(reponse.corps,"");
      strcat(reponse.corps, reponsePart1);
      strcat(reponse.corps, reponsePart2);
      strcat(reponse.corps, motsCles.mot);
      strcat(reponse.corps, reponsePart3);
      strcat(reponse.corps, motsCles.reponse);
      strcat(reponse.corps, reponsePart4);
    }
  }
  ...
  if (found == true)
  {
    ...
    printf("\n---------------------\n\nRéponse instantanée : ");
    printf("\n\nEmail : %s", reponse.email);
    printf("\n\nObjet : %s", reponse.obj);
    printf("\n\nCorps : \n%s", reponse.corps);
    fwrite(&reponse, sizeof(reponse), 1, sourceBdd);
  }
...
void envoyerMail()
{
  ...
  FILE *sourceBdd;
  sourceBdd = fopen("log_email_in.dat", "ab");
  if (sourceBdd == NULL)
  {
      fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
      exit(1);
  ...
  do {
    ...
    getchar();
    fgets(e.obj, sizeof(e.obj), stdin);
    removeNl(e.obj);

    printf("\nVeuillez entrer le crops : ");
    getchar();
    fgets(e.corps, sizeof(e.corps), stdin);
    removeNl(e.corps);

    fwrite(&e, sizeof(email), 1, sourceBdd);

    reponseMail(&e);

    do
      {
        printf("\nVoulez-vous envoyer un autre mail? (o/n) ");
        scanf(" %c", &choix);
...
void afficherMenuAdmin(char *mdp)
{
    ...
    do
    {
      ...
      if(checkIfDigit(choix) == false)
      {
       ...
       printf("9) Lister les mail reçus.\n");
       printf("10) Lister les mail envoyés automatiquement.\n");
       printf("\n0) Se deconnecter.\n\n\n");
       printf("Veuillez choisir une option parmis celles proposées dans le menu ci-dessus : ");
       scanf(" %s",choix);
       ...
       switch (atoi(choix))
       {
           ...
           case 8: supprimerMotCle(); afficherMenuAdmin(tmp_ptr_mdp);
               break;
           case 9: listerMailRecus(); afficherMenuAdmin(tmp_ptr_mdp);
               break;
           case 10: listerMailEnvoyes(); afficherMenuAdmin(tmp_ptr_mdp);
               break;
...
void saisirCoordonnes()
{
  ...
  FILE *sourceBdd;
  sourceBdd = fopen("coordonnees.dat", "ab");
  if (sourceBdd == NULL)
  {
      fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
      exit(1);
...
void listerCoordonnees()
{
  ...
  char choix;
  FILE *sourceBdd;
  sourceBdd = fopen("coordonnees.dat", "rb");
  if (sourceBdd == NULL)
  {
    fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
    exit(0);
...
void supprimerCoordonnee()
{
  ...
  tmp_sourceBdd = fopen("temp_coordonnees.dat", "ab");
  sourceBdd = fopen("coordonnees.dat", "rb");
...
void saisirMotCle()
{
  ...
  FILE *sourceBdd;
  sourceBdd = fopen("motcles.dat", "ab");
  if (sourceBdd == NULL)
  {
      fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
      exit(1);
...
void listerMotCle()
{
  ...
  char choix;
  FILE *sourceBdd;
  sourceBdd = fopen("motcles.dat", "rb");
  if (sourceBdd == NULL)
  {
    fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
    exit(0);
...
void supprimerMotCle()
{
  ...
  FILE *tmp_sourceBdd, *sourceBdd;
  tmp_sourceBdd = fopen("temp_motcles.dat", "ab");
  sourceBdd = fopen("motcles.dat", "rb");
  if (tmp_sourceBdd == NULL && sourceBdd == NULL)
  {
...
void listerMailRecus()
{
  ...
  char choix;
  FILE *sourceBdd;
  sourceBdd = fopen("log_email_in.dat", "rb");
  if (sourceBdd == NULL)
  {
    fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
    exit(0);
  ...
  while(fread(&e, sizeof(e), 1, sourceBdd))
  {
    printf("\nEmail : %s \nObjet : %s \nCorps : \n%s", e.email, e.obj, e.corps);
    printf("\n---------------------\n");
    compteur++;
  }

  printf("Email reçus en total : %d.", compteur);
  printf("\n---------------------\n\n");
...
void listerMailEnvoyes()
{
  ...
  char choix;
  FILE *sourceBdd;
  sourceBdd = fopen("log_email_out.dat", "rb");
  if (sourceBdd == NULL)
  {
    fprintf(stderr, "\nImpossible ouvrir la base de données!\n");
    exit(0);
  ...
  printf("Email envoyés en total : %d.", compteur);
  printf("\n---------------------\n\n");
